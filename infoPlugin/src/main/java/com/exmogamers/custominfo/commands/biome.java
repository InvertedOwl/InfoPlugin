...
public class biome implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (label.equalsIgnoreCase("biome")) {
            ...
            config.set(((Player) sender).getUniqueId() + ".biome", !config.getBoolean(((Player) sender).getUniqueId() + ".biome"));
            CustomInfo.getInstance().saveCustomYml(config);

            ((Player) sender).sendMessage("Biome toggle now " + config.getBoolean(((Player) sender).getUniqueId() + ".biome"));
